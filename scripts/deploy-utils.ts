...
/** Matches on-chain Inbox `MAX_ERROR_RETURN_DATA` — fee-template `errorLength` must not exceed this. */
 ...
 * Same floor as variable-fee deploy `gasPerByte` in inbox ship templates.
 */
 ...
 * Distinct from `FeeConfig.maxExecutionGas`, which must stay ≥ `constantFee` on-chain.
 */
export const CONSTANT_FEE_PRICED_EXECUTION_GAS = 12_000_000n;

/** Protocol ceiling for FeeConfig.maxMethodCallBytes (matches FeeManager.PROTOCOL_MAX_METHOD_CALL_BYTES). */
export const PROTOCOL_MAX_METHOD_CALL_BYTES = 32_768n;

/** Protocol ceiling for FeeConfig.maxExecutionGas (matches FeeManager.PROTOCOL_MAX_EXECUTION_GAS). */
export const PROTOCOL_MAX_EXECUTION_GAS = 25_000_000n;
...
export const assertConstantFeeCoversWorstCase = (fee: FeeConfigTuple, label = "FeeConfig"): void => {
  ...
  if (fee.maxExecutionGas < fee.constantFee) {
    throw new Error(
      `${label}: maxExecutionGas ${fee.maxExecutionGas} < constantFee ${fee.constantFee}. ` +
        `Set maxExecutionGas ≥ constantFee.`
    );
  }
};
...
/**
 * No-op. `maxMessageLife` / retry TTL were removed from Inbox.
 */
export const configureInboxMaxMessageLife = async (_params: {
  inbox: unknown;
  publicClient: unknown;
  walletClient: WalletClient;
  chainId: number;
...
> => {
  ...
  if (result.alreadyDeployed && mpcAbiReEncode !== zeroAddress) {
    const onChainCodec = getAddress((await inbox.read.mpcAbiReEncode()) as Address);
    if (onChainCodec !== mpcAbiReEncode) {
      throw new Error(
        `deployDeterministicInbox: Inbox at ${result.address} already deployed with mpcAbiReEncode=` +
          `${onChainCodec}, but deployed/expected codec is ${mpcAbiReEncode}. ` +
          `Remount the Inbox (bump deployConfig.inboxSalt.label) so init can wire the codec.`
      );
    }
  }
